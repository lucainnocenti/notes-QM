...
Any operator $A\in\Lin(\calX,\calY)$ defines a partition for $\calX$ and $\calY$, via its singular value decomposition. A more precise way to state this is that for any such $A$ there are collections of (not necessarily unit-trace) orthoprojectors,
\begin{equation}
	\{ \Pi^R_a: a\in\Sigma\}\subset \Proj(\calX),
	\qquad
	\{ \Pi^L_b: b\in\Sigma\}\subset \Proj(\calY),
\end{equation}
where $\Sigma$ is a finite register with $\lvert\Sigma\rvert\le\rank(A)$,
that are such that
\begin{equation}
	\Pi^L_a A \Pi^R_a = s(a) \Pi^L_a \Pi^R_a \simeq s(a) I_{\tr(\Pi^R_a)}.
\end{equation}
In other words, for all $a\in\Sigma$, $\Pi^L_aA\Pi^R_a$ is equivalent to a multiple of an identity operator, up to a change of input and output bases.
Here $s(a)\ge0$ are the singular values of $A$, and $|\Sigma|$ is equal to the number of \emph{distinct} singular values of $A$.
...
\begin{defn}[Contextual assignments]
...
% \begin{prop}
% 	% Denote with $\calE_t$ the channel describing attenuation through a beamsplitter with transmittivity $t$.
% 	\begin{equation}
% 		\langle j|
