...
\chapter{Maps}
Let $\rho\in\rmL(\calX)\simeq\calX\otimes\calX^*$ be a state.
A \emph{map} acting on $\rho$ is a linear operator $\Phi\in\rmL(\rmL(\calX),\rmL(\calY))$, sending states in $\calX$ into states in $\calY$.
Explicitly, we can write the action of $\Phi$ on $\rho$ in terms of their matrix components as
$\Phi(\rho)_{12}=K(\Phi)_{12,34}\rho_{34}$. The object $K(\Phi)$ is often referred to as the \emph{natural representation} of $\Phi$.
We can think of $K(\Phi)$ as an operator in $\rmL(\calX\otimes\calX^*,\calY\otimes\calY^*)$ mapping $\on{vec}(\rho)$ to $\on{vec}[\Phi(\rho)]$.
